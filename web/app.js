 ...
 *
 * Versao web: nao existe backend. O Stockfish roda no navegador de quem visita
 * (WebAssembly) e as partidas vem direto da API publica do chess.com.
 */

import { Engine, canUseThreads } from "./engine.js";
...
// O engine (7 MB de WebAssembly) so e baixado quando a primeira analise
// comeca — quem so quer ver a lista de partidas nao paga esse download.
let enginePromise = null;

function getEngine() {
  if (!enginePromise) {
    const badge = $("#engine-badge");
    badge.textContent = "baixando o Stockfish (7 MB)...";
    badge.className = "engine-badge";

    enginePromise = new Engine().start().then(
      (engine) => {
        badge.textContent = `${engine.name} · ${engine.threads} thread${engine.threads > 1 ? "s" : ""}`;
        badge.className = "engine-badge ok";
        return engine;
      },
      (err) => {
        badge.textContent = "falha ao carregar o Stockfish";
...
function showEngineIdle() {
  // Sem prometer multi-thread: so da para saber depois que o engine sobe.
  const badge = $("#engine-badge");
  badge.textContent = "Stockfish 18 · no seu navegador";
  badge.className = "engine-badge ok";
}

/* ---------------------------------------------------------------- cache */

// As revisoes ficam no navegador de quem visita. Reabrir a mesma partida na
// mesma profundidade e instantaneo, e nada disso sai da maquina do usuario.
const memCache = new Map();

function cacheKey(gameId, depth) {
  return `ca:v3:${gameId}:${depth}`;
}

function cacheGet(gameId, depth) {
  const key = cacheKey(gameId, depth);
  if (memCache.has(key)) return memCache.get(key);
  try {
    const raw = localStorage.getItem(key);
    if (!raw) return null;
    const review = JSON.parse(raw);
    memCache.set(key, review);
    return review;
  } catch {
    return null;
  }
...
function cacheSet(gameId, depth, review) {
  const key = cacheKey(gameId, depth);
  memCache.set(key, review);
  try {
    localStorage.setItem(key, JSON.stringify(review));
  } catch {
    // Estourou a cota: joga fora as revisoes antigas e tenta uma vez mais.
    try {
      for (const k of Object.keys(localStorage)) {
        if (k.startsWith("ca:v")) localStorage.removeItem(k);
      }
      localStorage.setItem(key, JSON.stringify(review));
    } catch {
      /* segue so com o cache em memoria */
    }
  }
}
...
$("#form-user").addEventListener("submit", async (ev) => {
  ...
  try {
    const games = await chesscom.listGames(username, Number(count), (found) => {
      $("#games-title").textContent = `Buscando partidas... (${found})`;
    });
...
$("#btn-pgn").addEventListener("click", () => {
  ...
  const hdr = chesscom.headersOf(pgn);
  const game = {
    id: "pgn-" + hashString(pgn),
    pgn,
    url: null,
    color: "white",
    opponent: hdr.Black || "?",
    result: "unknown",
    opening: chesscom.openingName(hdr),
    time_class: hdr.TimeControl || "",
    time_control: hdr.TimeControl || "",
    played_at: hdr.UTCDate || "",
  };
  // Partida colada entra na lista para o drill-down do lote achar depois.
  if (!state.games.some((g) => g.id === game.id)) state.games.push(game);
  startAnalysis(game);
});
...
async function startAnalysis(game, jumpToPly) {
  ...
  const cached = cacheGet(game.id, state.depth);
  if (cached) {
    showReview(cached);
    return;
  }
  ...
  try {
    ...
    const review = await analyzeGame(engine, game.pgn, game.color, {
      depth: state.depth,
      onProgress: ({ done, total }) => {
        $("#progress-fill").style.width = (total ? (done / total) * 100 : 0) + "%";
        $("#progress-text").textContent =
          `lance ${done} de ${total} (profundidade ${state.depth})`;
      },
    });
    cacheSet(game.id, state.depth, review);
    showReview(review);
  } catch (err) {
    $("#progress-text").textContent = err.message || "falha na análise";
  }
}
...
const batch = { canceled: false, running: false, startedAt: 0, analyzed: 0 };

$("#btn-batch").addEventListener("click", startBatch);
$("#btn-batch-cancel").addEventListener("click", cancelBatch);

async function startBatch() {
  if (!state.games.length || batch.running) return;
  ...
  try {
    for (let i = 0; i < games.length; i++) {
      ...
      const cached = cacheGet(game.id, depth);
      if (cached) {
        entries.push({ game, review: cached });
        onBatchProgress({ game: i + 1, done: 1, total: 1, cached: true, opponent: game.opponent },
          games.length);
        continue;
      }

      // Só abre o engine quando alguma partida realmente precisa ser analisada.
      const engine = await getEngine();
      const review = await analyzeGame(engine, game.pgn, game.color, {
        depth,
        onProgress: ({ done, total }) =>
          onBatchProgress({ game: i + 1, done, total, cached: false, opponent: game.opponent },
            games.length),
      });
      cacheSet(game.id, depth, review);
      entries.push({ game, review });
    }
  } catch (err) {
    $("#batch-text").textContent = err.message || "falha no lote";
    batch.running = false;
    return;
  }
  ...
  batch.running = false;
  if (!entries.length) {
    $("#batch-text").textContent = "Nenhuma partida foi analisada.";
    return;
  }
  showAggregate(agg.build(entries, state.username, depth), batch.canceled);
}
...
// existe para depurar pelo console do navegador.
